...
class PLP_analyzer():
    ...
    _methods = {"TFA", "TFA++", "SFA", "SFA++", "PLP", "PLP++"}
    ...
    def analyze(self, method:str="PLP++", lp_file:str="fifo.lp", use_tfa:bool=True, use_sfa:bool=True)->None:
        '''
        Analyse the stored network

        method: Allow methods are "TFA", "TFA++", "SFA", "SFA++", "PLP", "PLP++"
        ...
        # Analyse result
        if method.upper().startswith("PLP"):
            return self.analyze_plp(lp_file, use_tfa, use_sfa)
        if method.upper().startswith("TFA"):
            return self.analyze_tfa(lp_file)
        if method.upper().startswith("SFA"):
    ...
    def analyze_plp(self, lp_file:str="fifo.lp", use_tfa:bool=True, use_sfa:bool=True)->tuple:
        ...
        plp = FifoLP(self.network, tfa=use_tfa, sfa=use_sfa, filename=lp_file)
    ...
    def analyze_sfa(self, lp_file:str="fifo.lp")->tuple:
        ...
        delay_per_server = sfa.delay_servers
